...
class _OwnerEmailVerificationScreenState
  ...
  final _apiService = ApiService(); [cite: 15]
  ...
  Future<void> _verifyCode() async {
    ...
    final response =
        await _apiService.verifyOwnerEmail(widget.email, _codeController.text); [cite: 17]

    setState(() => _isLoading = false);
    if (!mounted) return; [cite: 18]
    ...
    if (response.statusCode == 200) {
      ...
      Navigator.pushReplacement(
          context,
          MaterialPageRoute(
              builder: (context) => const OwnerLoginScreen())); [cite: 19]
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
  ...
  Future<void> _resendCode() async {
    setState(() => _isLoading = true);
    final response =
        await _apiService.resendOwnerVerificationCode(widget.email); [cite: 22]

    setState(() => _isLoading = false);
    if (!mounted) return;
    if (response.statusCode == 200) { [cite: 23]
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
